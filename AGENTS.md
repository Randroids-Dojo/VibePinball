...
Shared rules for every agentic coding tool working in VibePinball. Claude Code, Codex, Cursor, and any future agent: this file is mandatory reading before you write anything.
...
Project pitch: A vibe coded pinball web-native game built while streaming.
...
The Game Design Document at `docs/gdd/` is the source of truth for what VibePinball is. Before proposing architecture, adding features, or changing data schemas, read it. If the GDD and your idea disagree, the GDD wins unless explicitly approved.
...
VibePinball is a 3D web-native game. The approved v1 stack is Next.js App Router, React, TypeScript strict, Three.js, Rapier 3D, Zod, Vitest, Playwright, Vercel, Upstash Redis for the global leaderboard, and `@randroids-dojo/vibekit` for shared game utilities.

Do not introduce new dependencies in core categories without explicit user approval. Rapier 3D is the approved physics dependency. Three.js is the approved renderer. Vibekit is the approved shared component and utility dependency.
